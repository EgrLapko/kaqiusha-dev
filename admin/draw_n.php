...
$query =  "SELECT * FROM  `artnew` WHERE nameru = '$art' ";
			...
			if ($row['gdraw']) $title = "../data/draw_n/".$row['draw']."/".$row['gdraw'];		
			$tabw = "new".$row['namelt'];
			                                          } 
$mdir = "../data/draw_n/".$edit['draw'];
$mdir2 = "$mdir/prodano";
...
<div class="content" name="content" style="height:800px">
	<div class="header">
		...
		<a href="art_new.php"><img alt="" class="logo" height="206" src="img/Logo.png" width="832" /></a></div>
		<div class="nav_l">	
						<p1>КАРТИНЫ&nbsp; <? echo "($num_rows)"; ?></p1>
<form name='sell' action='dell_n.php' method='post'>
   ...
   $table .= "<tr><td><input name='table' type='hidden' value='".$edit['namelt']."'/></td></tr>\n";
          while($row = mysql_fetch_assoc($resultall))
           { 
             ...
             $table.= "<td ><a href='draw_n.php?img=".$row['sell']."&id=".$row['id']."'><img style='max-height:85px;max-width:170px' src='$mdir/".$row['sell']."'></a></td></tr>\n";     
...
?>
			...
			<form action="setdraw_n.php" class="form-art" method="post" name="art">
				<table cellpadding="2" class="table" style="width: 100%">
				<tr> 
				<td style="width: 235px">
				<input name="namecn" type="hidden" value="<? echo $edit['namecn']; ?>"/></td>
				<td><input name="nameru" type="hidden" value="<? echo $edit['nameru']; ?>"/></td>
				<td><input name="namelt" type="hidden" value="<? echo $edit['namelt']; ?>"/></td>
				</tr><tr>
								<td><input name="table" type="hidden" value="new<? echo $edit['namelt']; ?>"/></td>
